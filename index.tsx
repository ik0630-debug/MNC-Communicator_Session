...
const App = () => {
  ...
  const [speakerTimerPreview, setSpeakerTimerPreview] = useState<TimerState>({
    initialTime: 60 * 60, timeRemaining: 60 * 60, isRunning: false
  });
  ...
  useEffect(() => {
    if (view === 'speaker_console') {
      ...
      worker.postMessage({ command: 'setTime', value: speakerTimerPreview.initialTime });
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (liveSpeakerTimer) {
            setLiveTimeDisplay(liveSpeakerTimer.timeRemaining);

            if (liveSpeakerTimer.isRunning && liveSpeakerTimer.timeRemaining > 0) {
  ...
  const handleStartPause = () => {
    if (speakerTimerPreview.timeRemaining > 0 || !speakerTimerPreview.isRunning) {
        ...
        setSpeakerTimerPreview(prev => ({ ...prev, isRunning: newIsRunning }));
        if (workerRef.current) {
  ...
  const handleSpeakerBroadcast = () => {
    if (isFirebaseConfigured && database && sessionId) {
      ...
      set(timerRef, speakerTimerPreview);
    }
  };
