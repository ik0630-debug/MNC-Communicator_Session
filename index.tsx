...
// GlobalConfigSettings: 전역으로 저장될 설정 항목들(스타일, 메시지 프리셋)만 명시적으로 포함합니다.
  ...
  const handleStyleChange = (group: keyof Styles | 'message' | 'timer' | 'image' | null, prop: string, value: string, isNested = true) => {
    if (isNested && group) {
        if (group === 'timer' || group === 'message' || group === 'image') {
            const numericProps = ['fontSize', 'blinkSpeed'];
            const processedValue = numericProps.includes(prop) ? parseFloat(value) : value;
            setLocalStyles(prev => ({ ...prev, [group]: { ...prev[group], [prop]: processedValue } }));
        }
    } else {
        ...
        <div className="modal-settings-pane">
            ...
            <div className="modal-controls">
               ...
               <fieldset>
                    ...
                    <div className="control-row">
                        <label htmlFor="blink-speed-slider">깜빡임 속도</label>
                        <div className="font-size-control">
                            <input
                                id="blink-speed-slider"
                                type="range"
                                min="0.5"
                                max="5"
                                step="0.1"
                                value={localStyles.message.blinkSpeed ?? 2}
                                onChange={(e) => handleStyleChange('message', 'blinkSpeed', e.target.value)}
                            />
                            <input
                                className="px-input"
                                type="number"
                                ...
                                value={localStyles.message.blinkSpeed ?? 2}
                                onChange={(e) => handleStyleChange('message', 'blinkSpeed', e.target.value)}
                                aria-label="Blink speed in seconds"
                            />
                            <span>s</span>
                        </div>
                    </div>
...
const TimerSettings = ({ minutes, seconds, setMinutes, setSeconds, onSetTime, onStartPause, onReset, isRunning }: TimerSettingsProps) => (
    <div className="control-group">
        ...
        <div className="timer-controls">
            ...
            <button className="primary" onClick={onSetTime}>시간 설정</button>
    ...
    <div className="panel console-panel">
      ...
      {!isModerator && (
          <TimerSettings 
              ...
              onSetTime={() => onSetTime(parseInt(minutes, 10) || 0, parseInt(seconds, 10) || 0)}
    ...
    const panelContent = () => {
        ...
        return (
            <>
                {showMessage && (
                    <span 
                        className={`speaker-text message-text ${isBlinking ? 'blinking' : ''}`} 
                        style={{ 
                            color: styles.message.color, 
                            fontSize: `${styles.message.fontSize}cqw`,
                            animationDuration: isBlinking ? `${styles.message.blinkSpeed ?? 2}s` : undefined
                        }}
                    >
                        {message}
                    </span>
...
const App = () => {
  ...
  const [styles, setStyles] = useState<Styles>({
    ...
    message: { color: '#FFFFFF', fontSize: 7.81, blinkSpeed: 2 }, // Default 150px on 1920px width
    image: { fit: 'contain' },
  });
  ...
  useEffect(() => {
    ...
    const isProtectedView = view === 'moderator_screen' || view === 'speaker_screen';

    if (isProtectedView) {
        window.addEventListener('beforeunload', handleBeforeUnload);
    }

    // Cleanup function to remove the listener when the component unmounts or the view changes.
    return () => {
        if (isProtectedView) {
        ...
        const onConfigsUpdate = onValue(configsRef, (snapshot) => {
            ...
            if (data) {
                const configsArray: SavedConfig[] = Object.entries(data).map(([name, settings]) => ({
                    name,
                    settings: settings as GlobalConfigSettings,
                }));
  ...
  const broadcastTimerState = (state: TimerState) => {
    if (isFirebaseConfigured && database && sessionId) {
        ...
        set(timerRef, { ...state, lastUpdatedTimestamp: serverTimestamp() });
    }
  };
  ...
  const handleSelectView = (selectedView: View, selectedSessionId: string) => {
      if (selectedView === 'moderator_screen' || selectedView === 'speaker_screen') {
  ...
  const handleSessionIdConfirm = (id: string) => {
      ...
      url.searchParams.set('session', lowercasedId);
      window.history.pushState({}, '', url);
      setSessionId(lowercasedId);
  };
  ...
  const handleSaveConfig = (name: string) => {
    ...
    // GlobalConfigSettings 타입에 맞춰 저장할 객체를 명시적으로 생성합니다.
    const currentSettings: GlobalConfigSettings = { styles, presetMessages };
  ...
  const handleLoadConfig = (name: string) => {
    ...
    if (configToLoad) {
        // Ensure backward compatibility with old configs by deep merging nested style objects
        const loadedStyles: Styles = {
            ...styles,
            ...configToLoad.settings.styles,
            message: {
                ...styles.message,
                ...(configToLoad.settings.styles?.message || {})
            },
            timer: {
              ...styles.timer,
              ...(configToLoad.settings.styles?.timer || {}),
              fontSizes: {
                ...styles.timer.fontSizes,
                ...(configToLoad.settings.styles?.timer?.fontSizes || {})
              }
            },
  ...
  if (view === 'moderator_screen') {
    return (
      <div className="fullscreen-view">
        <button className="fullscreen-close-button" onClick={handleCloseDoubleClick} aria-label="선택 화면으로 돌아가기">&times;</button>
        <SpeakerPanel
          title=""
          timeRemaining={liveTimeDisplay}
          message={liveModeratorContent?.message ?? ''}
